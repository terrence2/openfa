...
use lay::Layer;
use omnilib::OmniLib;
use na::{Isometry3, Matrix4, Orthographic3, Perspective3, Point3, Rotation3, Vector3};
use pal::Palette;
use std::{
    f32::consts::PI,
    path::{Path, PathBuf},
...
pub fn main() -> Fallible<()> {
    ...
    let base_palette = Palette::from_bytes(&omnilib.library("FA").load("PALETTE.PAL")?)?;

    let layers = omnilib.library("FA").find_matching("*.LAY")?;

    println!("LAYER: {}", layers[0]);
    let layer_bytes = omnilib.library("FA").load(&layers[0])?;
    let layer = Layer::from_bytes(&layer_bytes)?;

    let terrains = &omnilib.library("FA").find_matching("*.T2")?;

    let terrain = Terrain::from_bytes(&omnilib.library("FA").load(&terrains[0])?)?;
