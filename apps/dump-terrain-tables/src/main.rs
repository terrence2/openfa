...
impl PatchWinding {
    ...
    fn const_name(&self) -> String {
        match self {
            ...
            Self::Missing01 => "MISSING01",
            Self::Missing12 => "MISSING12",
...
fn make_line_list(
    ...
    if winding == PatchWinding::Full || bins.len() == 1 {
        for bin in bins.iter().rev() {
            // push left side of strip
            let tri = binned[bin].first().unwrap();
            indices.push(tri.i[0]);
            indices.push(tri.i[1]);
            for (i, tri) in binned[bin].iter().enumerate() {
                if i % 2 == 0 {
                    // upward facing tris
                    // Push bottom
                    indices.push(tri.i[1]);
        ...
        // Handle pairs of bins.
        assert!(bins.len() % 2 == 0);
        for pairs in bins.chunks(2) {
            let bin_a = pairs[0];
            let bin_b = pairs[1];

            // Push left side.
            if winding == PatchWinding::Missing0
                || winding == PatchWinding::Missing01
                || winding == PatchWinding::Missing20
            {
                panic!("wind at half resolution")
            } else {
                panic!("asdf")
                let tri_a = binned[bin_a].first().unwrap();
                let tri_b = binned[bin_a].first().unwrap();
