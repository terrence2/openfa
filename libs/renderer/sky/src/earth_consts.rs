...
fn interpolate(wavelengths: &[f64], properties: &[f64], wavelength: f64) -> f64 {
...
fn interpolate_rgb(wavelengths: &[f64], properties: &[f64], scale: f64) -> [f32; 3] {
    [
        (interpolate(wavelengths, properties, 680.0) * scale) as f32,
        (interpolate(wavelengths, properties, 550.0) * scale) as f32,
        (interpolate(wavelengths, properties, 440.0) * scale) as f32,
    ]
}
...
impl fs::ty::AtmosphereParameters {
    pub fn earth() -> Self {
        // Build a physical model of earth's atmosphere.
        ...
        const SOLAR_IRRADIANCE: [f64; 48] = [
            1.11776, 1.14259, 1.01249, 1.14716, 1.72765, 1.73054, 1.6887, 1.61253, 1.91198,
            2.03474, 2.02042, 2.02212, 1.93377, 1.95809, 1.91686, 1.8298, 1.8685, 1.8931, 1.85149,
            1.8504, 1.8341, 1.8345, 1.8147, 1.78158, 1.7533, 1.6965, 1.68194, 1.64654, 1.6048,
            1.52143, 1.55622, 1.5113, 1.474, 1.4482, 1.41018, 1.36775, 1.34188, 1.31429, 1.28303,
        ...
        const RAYLEIGH_SCATTER_COEFFICENT: f64 = 1.24062e-6;
            ...
            .zip(OZONE_CROSS_SECTION.iter())
        {
            ...
            rayleigh_scattering.push(RAYLEIGH_SCATTER_COEFFICENT * lambda.pow(-4.0));
        ...
        Self {
            sun_irradiance: interpolate_rgb(&wavelengths, &sun_irradiance, 1.0),
            ...
            rayleigh_density: fs::ty::DensityProfile {
                layers: [
                    fs::ty::DensityProfileLayer {
                        width: 0f32,
                        exp_term: 1f32,
                        exp_scale: -1f32 / 8000f32,
                        linear_term: 0f32,
                        constant_term: 0f32,
                        _dummy0: Default::default(),
                    },
            ...
            rayleigh_scattering_coefficent: interpolate_rgb(
                &wavelengths,
                &rayleigh_scattering,
                LENGTH_SCALE,
            ),
            ...
            mie_scattering_coefficient: interpolate_rgb(
                &wavelengths,
                &mie_scattering,
                LENGTH_SCALE,
            ),
            mie_extinction_coefficient: interpolate_rgb(
                &wavelengths,
                &mie_extinction,
                LENGTH_SCALE,
            ),
            ...
            absorption_extinction_coefficient: interpolate_rgb(
                &wavelengths,
                &absorption_extinction,
                LENGTH_SCALE,
            ),
            ground_albedo: interpolate_rgb(&wavelengths, &ground_albedo, 1.0),
