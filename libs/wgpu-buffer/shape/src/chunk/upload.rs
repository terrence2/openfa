...
#[derive(Clone)]
pub(crate) struct BufferProps {
    context: String,
    pub(crate) flags: VertexFlags,
    pub(crate) xform_id: u32,
}
...
impl<'a> ShapeUploader<'a> {
    ...
    fn load_vertex_buffer(
        ...
        for v in vert_buf.vertices() {
            ...
            self.vert_pool.push(ShapeVertex::new(&position, &props));
        }
        trace!(
    ...
    fn push_facet(&mut self, facet: &Facet, override_flags: Option<VertexFlags>) -> Result<()> {
        ...
        for i in 2..facet.indices.len() {
            ...
            for j in &js {
                ...
                v.set_color(self.palette.rgba_f32(facet.color as usize)?);
    ...
    pub(crate) fn draw_model(
        ...
        gpu: &Gpu,
    ) -> Result<(Arc<RwLock<ShapeMetadata>>, Vec<ShapeVertex>)> {
        ...
        let mut callback = |_pc: &ProgramCounter, instr: &Instr| {
            match instr {
                ...
                Instr::VertexNormal(vert_norm) => {
                    ...
                    .normalize();
                    self.vert_pool[vert_norm.index].set_is_vertex_normal();
                    self.vert_pool[vert_norm.index].set_normal([n.x, n.y, n.z]);
                }
