...
impl ShapeInstance {
    fn new(buffer: Rc<ShapeBuffer>) -> Fallible<Self> {
        Ok(Self {
            buffer,
    ...
    fn render(
        ...
        let (vb, ib) = buffer_manager.buffers_at(&self.buffer.get_pointer());

        let bufs = [DrawIndexedIndirectCommand {
            index_count: self.buffer.get_pointer().index_count as u32,
            first_index: 0,
            vertex_offset: 0,
            first_instance: 0,
            instance_count: 1,
